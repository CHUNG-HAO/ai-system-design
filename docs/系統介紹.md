...
本專題由 **6 位成員** 共同完成，一位負責口頭報告，其餘五位依模組分工：

| 角色 | 學號 / 系所 / 姓名 | 主要任務 | 對應檔案 |
|---|---|---|---|
| **🎤 簡報與報告** | 313615017　環境工程學系碩二　**李求鎮** | 製作簡報、彙整故事線、上台 demo、回答評審問題 | 簡報、本文件 |
| **🧠 後端架構與資料層** | 114481030　人社系大一　**陳宏威** | FastAPI 骨架、CSV 載入、案件查詢 API、CORS / port 設計 | `poc/backend/main.py`、`poc/backend/data.py` |
| **🤖 LLM + Prompt 工程** | 314704041　工業工程管理系碩一　**洪意婷** | Qwen 7B 包裝、prompt 模板設計、JSON 解析、規則引擎 fallback | `poc/backend/llm.py`、`poc/backend/prompts.py` |
| **🔍 RAG 知識庫設計** | 314612506　土木工程碩一　**鍾弘浩** | Corpus 設計（5 個 protocol + 200 件歷史案例）、bge embedding 索引建立、查詢策略、cache 機制 | `poc/backend/rag.py` |
| **🎨 前端 UI / UX** | 312704036　工業工程管理系碩三　**林懌岑** | Next.js App Router、案件列表 / 詳情頁、生命徵象 recharts、SBAR 終端機風格、`/intake` 多模態頁 | `poc/frontend/app/`、`poc/frontend/components/`、`lib/api.ts` |
| **📸 多模態整合（Vision + Audio）** | 313593012  ICST Vineeth Jacob 建國 | Qwen2-VL 拍照分析、Whisper 語音轉錄、opencc 繁簡處理、`MediaRecorder` 錄音、檔案上傳流程 | `poc/backend/vision.py`、`poc/backend/audio.py`、`poc/frontend/app/intake/page.tsx` |

每個模組成員另負責：
- 對應模組的 fallback 規則 / 測試
- 自己模組的 5 個 demo case 驗證
